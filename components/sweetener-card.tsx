...
export function SweetenerCard({ sweetener, activeType }: SweetenerCardProps) {
  ...
  return (
    <div className="bg-card border-border hover:bg-accent/40 flex w-full items-center gap-2 rounded-2xl border p-2 pr-1 text-left transition-all duration-150">
      <Link
        href={`/sweetener/${sweetener.id}`}
        className="focus-visible:ring-ring flex min-w-0 flex-1 items-center gap-4 rounded-xl p-2 focus-visible:ring-2 focus-visible:outline-none active:scale-[0.98]"
